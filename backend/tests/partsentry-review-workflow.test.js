...
// Current-repo convention (see provider-platform.test.js): the service imports the shared
// auditLogger, which imports db/supabase.js — that module throws at import time without these
// env vars, so they must be set BEFORE the (dynamic) service import. The tests themselves only
// ever use the in-memory mock client passed explicitly to each service call.
...
} = await import('../services/trustGovernance/partsentryReviewService.js');
...
test('approving public_card_eligible stamps reviewer provenance on partsentry_logs', async () => {
  const client = createMockClient();
  const request = await createPartSentryReviewRequest(client, mechanic, 101, {
    request_type: 'public_card_eligible',
    requested_value: { public_card_eligible: true },
  ...
  await approvePartSentryReviewRequest(client, admin, request.id, {
    decision_notes: 'Provenance must be stamped',
  });

  const log = client.data.partsentry_logs.find(row => row.id === 101);
  assert.equal(log.public_card_eligible, true);
  // approved_by/approved_at are what main's summarizePartSentry needs to treat the
  // approval as provenanced (otherwise the public card renders as review_required),
  // and approved_by must be the REVIEWER (not the submitting mechanic) so the
  // read-side self-approval guard (approved_by === mechanic_id) stays meaningful.
  assert.equal(log.approved_by, 'admin-1');
  assert.notEqual(log.approved_by, log.mechanic_id);
  assert.ok(log.approved_at, 'approved_at must be stamped');
  const approvedAt = Date.parse(log.approved_at);
  assert.ok(Number.isFinite(approvedAt) && approvedAt >= before - 1000, 'approved_at must be a fresh timestamp');
});
...
test('unknown suspicion_status blocks approval (fail-closed allowlist, not watch/flagged denylist)', async () => {
  const client = createMockClient();
  const request = await createPartSentryReviewRequest(client, mechanic, 101, {
    request_type: 'public_card_eligible',
    requested_value: { public_card_eligible: true },
  ...
  // A future/unknown moderation state must be treated as suspicious (main's allowlist
  // semantics: only none/cleared/'' are non-suspicious), so the approval is blocked
  // even though the value is neither 'watch' nor 'flagged'.
  client.data.partsentry_logs.find(row => row.id === 101).suspicion_status = 'under_review';

  await assert.rejects(
    () => approvePartSentryReviewRequest(client, admin, request.id, { decision_notes: 'Approve unknown suspicion state' }),
    /blocked/
  );
  const log = client.data.partsentry_logs.find(row => row.id === 101);
  assert.equal(log.public_card_eligible, false);
  assert.equal(log.approved_by ?? null, null);
});
