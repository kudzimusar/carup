...
const shareOverrideTampering = Object.freeze({
  baseUrl: 'https://carup.dev',
  base_url: 'https://carup.dev',
  publicAppUrl: 'https://carup.dev',
  public_app_url: 'https://carup.dev',
  PUBLIC_APP_URL: 'https://carup.dev',
  VITE_PUBLIC_APP_URL: 'https://carup.dev',
  CARUP_PUBLIC_BASE_URL: 'https://carup.dev',
  CARUP_PUBLIC_APP_URL: 'https://carup.dev',
  env: { PUBLIC_APP_URL: 'https://carup.dev' },
...
test('share assets resolve public app URL by trusted environment, not staging user input', () => {
  assert.equal(resolveReferralPublicAppUrl({}, { CARUP_PUBLIC_BASE_URL: 'https://carup.dev' }), 'https://carup.dev');
  ...
  assert.equal(stagingAssets.short_referral_url, 'https://staging.carup.dev/r/STAGE-CODE-50');
  // Must be the staging subdomain, not the bare production origin (a plain .includes('carup.dev')
  // would false-pass here since 'staging.carup.dev' contains 'carup.dev' as a substring).
  assert.equal(stagingAssets.short_referral_url.startsWith('https://carup.dev'), false);
...
test('request-supplied share options cannot override the trusted public app URL', async () => {
  const { repository, service } = createService();
  service.shareOptions = { baseUrl: 'https://staging.carup.dev' };
  ...
  assert.equal(asset.payload.short_referral_url, 'https://staging.carup.dev/r/STAGE-SAFE-CODE');
