...
// Finding 6: one attributed inquiry must produce exactly one code_validated event and one lead event
// (the bridge pre-check uses record:false; createLead does the single authoritative validation).
test('finding 6: one attributed inquiry yields exactly one code_validated and one lead_created event', async () => {
  const { repository, bridge, code } = await createHarness();
  const inquiry = makeInquiry({ referral_code: code.code });
  await bridge.bridgeInquiryToReferralLead({ inquiry, actor: inviteeActor });
  const validations = await repository.list(REFERRAL_TABLES.events, { event_type: 'referral.code_validated' });
  const leads = await leadEventsFor(repository, inquiry.id);
  assert.equal(validations.length, 1, 'exactly one code_validated event (no duplicate from the pre-check)');
  assert.equal(leads.length, 1, 'exactly one lead event');
});

// Finding 5: atomic idempotency under CONCURRENT execution. A constraint-enforcing repository models the
// partial unique index on (source_inquiry_id) for lead events; two concurrent bridges must converge to one.
class ConstraintMemoryRepository extends MemoryReferralRepository {
  async findOne(table, filters = {}) { await Promise.resolve(); return super.findOne(table, filters); }
  async insert(table, payload) {
    await Promise.resolve(); // yield so concurrent callers interleave past the check-then-insert window
    if (
      table === REFERRAL_TABLES.events &&
      payload.event_type === LOCAL_MARKETPLACE_EVENT_TYPES.LEAD_CREATED &&
      payload.metadata && payload.metadata.source_inquiry_id
    ) {
      const clash = this.tables.get(table).find(
        (row) => row.event_type === LOCAL_MARKETPLACE_EVENT_TYPES.LEAD_CREATED &&
          (row.metadata || {}).source_inquiry_id === payload.metadata.source_inquiry_id
      );
      if (clash) { const err = new Error('duplicate key value violates unique constraint'); err.code = '23505'; throw err; }
    }
    return super.insert(table, payload);
  }
}

test('finding 5: concurrent bridging of the same inquiry creates exactly one lead (atomic idempotency)', async () => {
  const repository = new ConstraintMemoryRepository();
  const now = () => new Date('2026-07-15T00:00:00.000Z');
  const referralService = new ReferralEngineService({ repository, now });
  const bridge = new MarketplaceReferralBridgeService({ referralService });
  const campaign = await referralService.createCampaign({ name: 'S4 Concurrency', campaign_type: 'LOCAL_MARKETPLACE', priority_scope: 'LOCAL', status: 'ACTIVE' }, operatorActor);
  const code = await referralService.createReferralCode({ campaign_id: campaign.id, owner_user_id: OWNER, code: 'S4CONC001', code_type: REFERRAL_CODE_TYPES.MEMBER, channel: 'web' }, operatorActor);
  const inquiry = makeInquiry({ id: 'inq-conc', referral_code: code.code });

  const results = await Promise.all([
    bridge.bridgeInquiryToReferralLead({ inquiry, actor: inviteeActor }),
    bridge.bridgeInquiryToReferralLead({ inquiry, actor: inviteeActor }),
  ]);

  assert.ok(results.every((r) => r.bridged), 'both concurrent calls resolve to a lead');
  const leads = await leadEventsFor(repository, inquiry.id);
  assert.equal(leads.length, 1, 'exactly one lead survives concurrent creation');
  assert.equal(results[0].lead_event_id, results[1].lead_event_id, 'both calls reference the same single lead');
});
