...
// A reachable owner: listing eligibility refuses fixture VINs and seed owner ids, and execute
// now re-derives it, so a synthetic actor could no longer reach the behaviour under test.
const C_OWNER = { id: '7c9e6679-7425-40de-944b-e07fc1f90ae7', role: 'owner', platformRole: 'owner' };
...
function strictClient(seed = {}) {
  ...
  // An unknown table answers like real Postgres for a user with no rows: an EMPTY SET. Execute
  // now consults the workbook catalogue, which reads dealer-onboarding and trade-profile tables.
      ...
      if (state.insertRows) {
        if (table === 'diaspora_workbook_import_receipts') {
          for (const row of state.insertRows) {
            const clash = tableOf(table).some((existing) =>
...
function seededBatch({ receipts = [], importStatus = 'VALIDATED', evidence = null } = {}) {
  ...
  return strictClient({
    batches: [{ id: batchId, uploaded_by: C_OWNER.id, template_type: 'seller_vehicles', import_status: importStatus }],
    rows: [{
...
test('C4: a vehicle with evidence writes ONE receipt for its row — evidence no longer collides with it', async () => {
  const client = seededBatch({ evidence: [{ evidence_class: 'registration', evidence_subtype: 'registration_book', file_url: 'https://x/y.pdf', file_mime_type: 'application/pdf' }] });
  const result = await executeVehicleWorkbookImport(
    { batchId: 'batch-1', confirm: true }, C_OWNER,
...
test('C4: the evidence outcome is still reported per item — it is recorded, not discarded', async () => {
  ...
  const result = await executeVehicleWorkbookImport(
    { batchId: 'batch-1', confirm: true }, C_OWNER, { supabaseClient: client, dispatch });
...
test('C5: a third pass keeps counting — attempt is the pass number, and history is additive', async () => {
  ...
  await executeVehicleWorkbookImport({ batchId: 'batch-1', confirm: true }, C_OWNER,
    { supabaseClient: client, dispatch: realVehiclesAddDispatch });
  const attempts = client._db.diaspora_workbook_import_receipts
    .filter((r) => Number(r.row_number) === 1).map((r) => Number(r.attempt)).sort();
...
test('C7: the real create response carries no vehicle uuid, so no VIN is written to a uuid column', async () => {
  ...
  assert.equal(row.workbook_record_id, 'JTMHY7AJ2K4012345', 'the row stays linked by its VIN');
  const receipt = client._db.diaspora_workbook_import_receipts[0];
  assert.equal(receipt.entity_ref, 'JTMHY7AJ2K4012345', 'and the receipt still names the vehicle');
