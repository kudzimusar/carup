...
// ---------------------------------------------------------------------------
// B2 — Fixture-based tests using real synthetic image files from disk
// ---------------------------------------------------------------------------
...
test('fixture: valid JPEG from disk passes Layer 1', async () => {
  const front = readFixture('blank-white-doc.jpg');
  const selfie = readFixture('selfie.png');
  const result = await DocumentClassifier.classify({ front, selfie }, 'passport');
  assert.equal(result.extractionAllowed, true, 'valid JPEG should pass Layer 1');
  assert.equal(result.provider, 'mock');
  assert.equal(result.classification, EVIDENCE_CLASSIFICATION.VALID_IDENTITY_DOCUMENT);
});

test('fixture: valid PNG from disk passes Layer 1', async () => {
  const front = readFixture('synthetic-id.png');
  const selfie = readFixture('selfie.png');
  const result = await DocumentClassifier.classify({ front, selfie }, 'national_id');
  assert.equal(result.extractionAllowed, true, 'valid PNG should pass Layer 1');
});

test('fixture: tiny JPEG fails Layer 1 as too small', async () => {
  const front = readFixture('tiny-image.jpg');
  const result = await DocumentClassifier.classify({ front }, 'passport');
  assert.equal(result.extractionAllowed, false);
  assert.equal(result.classification, EVIDENCE_CLASSIFICATION.NON_DOCUMENT);
  assert.equal(result.reasonCode, 'DOCUMENT_TOO_SMALL');
});

test('fixture: corrupted binary file fails Layer 1', async () => {
  const front = readFixture('corrupted.bin');
  const result = await DocumentClassifier.classify({ front }, 'passport');
  assert.equal(result.extractionAllowed, false);
  assert.equal(result.classification, EVIDENCE_CLASSIFICATION.NON_DOCUMENT);
  assert.match(result.reasons[0], /not a supported/);
});

test('fixture: duplicate files caught by hash comparison', async () => {
  const same = readFixture('duplicate-id.png');
  const front = readFixture('synthetic-id.png'); // same bytes as duplicate-id.png
  const selfie = readFixture('selfie.png');
  const result = await DocumentClassifier.classify({ front, back: same, selfie }, 'national_id');
  assert.equal(result.extractionAllowed, false);
  assert.equal(result.reasonCode, 'FRONT_BACK_DUPLICATE');
});

test('fixture: cup-object from disk passes Layer 1 and is mock-classified', async () => {
  const result = await DocumentClassifier.classify(
    { front: readFixture('cup-object.jpg'), selfie: readFixture('selfie.png') },
    'passport',
  );
  assert.equal(result.extractionAllowed, true);
  assert.equal(result.provider, 'mock');
...
test('fixture: desk-object from disk passes Layer 1', async () => {
  const result = await DocumentClassifier.classify(
    { front: readFixture('desk-object.jpg'), selfie: readFixture('selfie.png') },
    'passport',
  );
  assert.equal(result.extractionAllowed, true);
  assert.equal(result.extractionTrust, EXTRACTION_TRUST_STATUS.PARTIALLY_TRUSTED);
...
test('fixture: overexposed document passes Layer 1', async () => {
  const result = await DocumentClassifier.classify(
    { front: readFixture('overexposed-doc.jpg'), selfie: readFixture('selfie.png') },
    'driver_license',
  );
  assert.equal(result.extractionAllowed, true);
});
