  ...
  const base = {
    ...
    sheets: {
      ...
      DIASPORA_IMPORT_ORDERS: [
        {
          ...
          BUYER_TRADE_PROFILE_ID: 'TP-BUYER-1',
      ...
      IMPORT_QUOTES: [
        {
          QUOTE_ID: 'Q-1',
          IMPORT_ORDER_ID: 'DIO-1',
          SELLER_TRADE_PROFILE_ID: 'TP-SELLER-1',
          QUOTE_AMOUNT: 350,
          QUOTE_CURRENCY: 'USD',
          STATUS: 'SUBMITTED',
        },
      ...
      CONTAINER_SHIPMENTS: [
        {
          ...
          BOOKING_DEADLINE: '2026-07-01',
          CONTAINER_TYPE: '40HQ',
          TOTAL_CAPACITY_VOLUME: 10,
          STATUS: 'BOOKING_OPEN',
        },
      ],
      CARGO_RESERVATIONS: [
      ...
      AI_COMMAND_CENTER: [
        {
          COMMAND_ID: 'CMD-1',
          RAW_COMMAND: 'Create a parts import order for Harare.',
          INTENT: 'CREATE_IMPORT_ORDER',
          RISK_LEVEL: 'LOW',
...
test('Dry-run validator is a no-write service and does not import Supabase client', () => {
  assert.equal(validationServiceFile.includes("from '../../db/supabase"), false);
  assert.equal(validationServiceFile.includes("from('../db/supabase"), false);
  assert.equal(validationServiceFile.includes('.from('), false);
  assert.equal(syncServiceFile.includes('wroteToDatabase: false'), false, 'write flag is produced by validation result, not hard-coded import side effects');
  assert.equal(templateServiceFile.includes('sourceOfTruth'), true);
});

test('Valid enterprise workbook dry-run succeeds and writes zero database records', () => {
  ...
  const workbook = validEnterpriseWorkbook({
    sheets: {
      DIASPORA_IMPORT_ORDERS: [
        {
          IMPORT_ORDER_ID: 'DIO-1',
          BUYER_TRADE_PROFILE_ID: 'MISSING-BUYER',
          ORDER_TYPE: 'parts_import',
          ORIGIN_COUNTRY: 'Japan',
          DESTINATION_COUNTRY: 'Zimbabwe',
          STATUS: 'SHIPPED_DIRECTLY_WITHOUT_WORKFLOW',
          BUDGET_CURRENCY: 'USD',
        },
      ],
    },
  ...
  const workbook = validEnterpriseWorkbook({
    sheets: {
      AI_COMMAND_CENTER: [
        {
          COMMAND_ID: 'CMD-1',
          RAW_COMMAND: 'Release escrow for this order.',
...
test('Phase 1A import/export/drive actions remain intentionally disabled', async () => {
  await assert.rejects(
    () => import('../services/diaspora/diasporaWorkbookSyncService.js').then((module) => module.importDiasporaWorkbook(validEnterpriseWorkbook(), { id: 'user-1' })),
    ValidationError,
  );
  await assert.rejects(
    () => import('../services/diaspora/diasporaWorkbookSyncService.js').then((module) => module.exportDiasporaWorkbook()),
    ValidationError,
  );
  await assert.rejects(
    () => import('../services/diaspora/diasporaWorkbookSyncService.js').then((module) => module.saveDiasporaWorkbookToDrive()),
    ValidationError,
  );
});
