 ...
 *
 *   POST /api/vehicles/:vin/escrow                 request transaction/escrow intent
 *   GET  /api/vehicles/:vin/escrow                 participant-scoped sessions
 *   GET  /api/escrow/:id                           participant-scoped session + history
 *   PATCH /api/escrow/:id/transition               request a permitted state transition
 *   POST /api/escrow/webhook                       signed sandbox provider reconciliation
 *
 * Buyer/seller/listing terms and eligibility facts are resolved server-side. Request bodies do
 * not get to assert counterparties, prices, gate verdicts, or provider-confirmed money states.
 */
...
async function serverGateContextFor(vin) {
  const decision = await getTrustDecision(vin);
  return buildMarketplaceEscrowGateContext(decision);
}

router.post('/api/vehicles/:vin/escrow', authorizeRole(['buyer', 'owner', 'dealer', 'admin']), async (req, res, next) => {
  try {
    const session = await requestMarketplaceEscrow(req.params.vin, {
      actor: actorFrom(req),
...
router.patch('/api/escrow/:id/transition', authorizeRole(['buyer', 'owner', 'dealer', 'admin', 'reviewer']), async (req, res, next) => {
  try {
    ...
    const gateContext = await serverGateContextFor(current.vin);
    const session = await transitionEscrow(req.params.id, req.body?.to_status, {
      actor,
      reason: req.body?.reason,
      gateContext,
    });
    res.json({ session: toPublicMarketplaceEscrowSession(session) });
  } catch (err) {
    next(err);
  }
});
