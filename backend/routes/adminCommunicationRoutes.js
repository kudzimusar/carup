...
export async function sendProviderSmokeTest({ services, channel = 'whatsapp', to, message, actor = {}, clientMessageId = null } = {}) {
  ...
  // Refuse fake-adapter "success". A fake adapter reports mode: 'fake'; only a real provider
  // adapter reports mode: 'real'. This is the single most important guard in this endpoint.
  if (config.mode !== 'real') {
    const err = new Error(`Refusing smoke test: channel '${normalizedChannel}' is served by a '${config.mode || 'non-real'}' adapter (provider=${config.provider || 'unknown'}). A real provider adapter is required — fake-adapter success is not accepted.`);
    err.statusCode = 409;
