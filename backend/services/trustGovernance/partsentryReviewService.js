...
/**
 * Fail-closed ALLOWLIST of non-suspicious states (aligned with main's
 * summarizePartSentry semantics): a log is treated as under active suspicion
 * unless its suspicion_status is one of these known-safe values (or absent).
 * Any unknown/future value (e.g. 'under_review') therefore BLOCKS approvals,
 * instead of the original denylist (watch/flagged) which would silently allow it.
 */
const NON_SUSPICIOUS_SUSPICION_STATUSES = ['none', 'cleared', ''];
...
function isActiveSuspicion(value) {
  ...
  return !NON_SUSPICIOUS_SUSPICION_STATUSES.includes(normalized);
}
...
export async function approvePartSentryReviewRequest(supabaseClient, actorInput, requestId, payload = {}, requestContext = {}) {
  ...
  if (request.request_type === 'public_card_eligible') {
    // Reviewer provenance stamp: main's read-side summary (summarizePartSentry) only
    // publishes public-card claims when approver provenance exists and the approver is
    // not the submitting mechanic (self-approval guard on approved_by/mechanic_id).
    // Without this stamp an approval renders as review_required instead of eligible.
    logPatch.approved_by = actor.id;
    logPatch.approved_at = new Date().toISOString();
  }
