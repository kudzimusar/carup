...
const PUBLIC_SAFE_TOOLS = new Set([
  AGENT_TOOL_NAMES.TRIAGE,
  AGENT_TOOL_NAMES.VALIDATE_REFERRAL_CODE,
  AGENT_TOOL_NAMES.CHECK_CAMPAIGN_STATUS,
  AGENT_TOOL_NAMES.DRAFT_SOCIAL_COPY,
]);

const WRITE_TOOLS = new Set([
  AGENT_TOOL_NAMES.DRAFT_CAMPAIGN,
  AGENT_TOOL_NAMES.GENERATE_SHARE_KIT,
  AGENT_TOOL_NAMES.SUPPORT_HANDOFF,
]);
...
const TOOL_ALIASES = Object.freeze({
  validate: AGENT_TOOL_NAMES.VALIDATE_REFERRAL_CODE,
  validate_code: AGENT_TOOL_NAMES.VALIDATE_REFERRAL_CODE,
  referral_validate: AGENT_TOOL_NAMES.VALIDATE_REFERRAL_CODE,
  campaign_status: AGENT_TOOL_NAMES.CHECK_CAMPAIGN_STATUS,
  share_kit: AGENT_TOOL_NAMES.GENERATE_SHARE_KIT,
  social_copy: AGENT_TOOL_NAMES.DRAFT_SOCIAL_COPY,
  wallet_status: AGENT_TOOL_NAMES.EXPLAIN_WALLET,
  handoff: AGENT_TOOL_NAMES.SUPPORT_HANDOFF,
  support: AGENT_TOOL_NAMES.SUPPORT_HANDOFF,
});

function normalizeToolName(tool) {
  const normalized = String(tool || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
...
function isAdmin(actor = {}) {
  return ADMIN_ROLES.has(normalizeRole(actor.actor_role || actor.role));
}

function isOperator(actor = {}) {
  return OPERATOR_ROLES.has(normalizeRole(actor.actor_role || actor.role));
}

function isTrustedActor(actor = {}) {
  return Boolean(actor.gateway_trusted || actor.actor_user_id || actor.user_id || isAdmin(actor));
}
...
function inferIntent(message = '') {
  ...
  const matches = [];
  if (/\b(code|referral|coupon|discount)\b/.test(text)) matches.push('referral_validation');
  if (/\b(wallet|reward|balance|paid|payout|credit)\b/.test(text)) matches.push('wallet_explanation');
  if (/\b(campaign|container|import|vehicle|parts|supplier|share)\b/.test(text)) matches.push('campaign_or_share');
  if (/\b(help|support|human|agent|problem|complaint|stuck)\b/.test(text)) matches.push('support_handoff');
  if (matches.length === 0) matches.push('general_triage');
  return matches[0];
}
...
function summarizeWallet(wallet, transactions = []) {
  ...
  const latest = transactions.slice(0, 5).map((tx) => ({
    id: tx.id,
    status: tx.status,
...
function buildCampaignDraft(input = {}) {
  ...
  return {
    ...
    status: REFERRAL_CAMPAIGN_STATUSES.DRAFT,
    starts_at: input.starts_at || null,
    ends_at: input.ends_at || null,
    route_origin: input.route_origin || null,
    route_destination: input.route_destination || null,
    category: input.category || null,
    channel_strategy: cleanObject(input.channel_strategy || {
      primary: input.channel || REFERRAL_CHANNELS.WEB,
      recommended_tools: [AGENT_TOOL_NAMES.GENERATE_SHARE_KIT, AGENT_TOOL_NAMES.DRAFT_SOCIAL_COPY],
    }),
    metadata: cleanObject({
      ...input.metadata,
      drafted_by: 'agent_gateway',
      requires_human_review: true,
      source_message: input.message || null,
    }),
  };
}
...
function buildSocialCopy(input = {}, validation = null) {
  ...
  const copyBase = code
    ? `Use my CarUp code ${code} for ${campaignLabel}.`
    : `Join me on CarUp for ${campaignLabel}.`;
  return {
    whatsapp: `${copyBase} Ask for verified local buying, selling, parts, imports, or container-space support.`,
    telegram: `${copyBase} CarUp keeps referral attribution and support history in one place.`,
    short_post: `${copyBase} CarUp helps connect buyers, sellers, suppliers, importers, and agents with a traceable referral path.`,
    compliance_note: 'Draft copy only. Human review is required before paid promotion, reward promises, or regulated claims.',
  };
}

export class ReferralAgentGatewayService {
  constructor({ referralService, client, now = () => new Date(), shareOptions = {} } = {}) {
    this.referralService = referralService || new ReferralEngineService({ client, now, shareOptions });
    this.now = now;
    this.shareOptions = shareOptions;
  }

  getToolCatalog(actor = {}) {
    return Object.values(AGENT_TOOL_NAMES).map((name) => ({
      name,
      permission: this.describePermission(name, actor),
      writes: WRITE_TOOLS.has(name),
      available: this.isToolVisible(name, actor),
    }));
  }

  describePermission(tool, actor = {}) {
    const name = normalizeToolName(tool);
    if (PUBLIC_SAFE_TOOLS.has(name)) return 'public_safe';
    if (name === AGENT_TOOL_NAMES.EXPLAIN_WALLET) return 'self_or_admin_read';
    if (name === AGENT_TOOL_NAMES.DRAFT_CAMPAIGN) return 'dry_run_public_persist_operator';
    if (name === AGENT_TOOL_NAMES.GENERATE_SHARE_KIT) return 'dry_run_public_persist_operator';
    if (name === AGENT_TOOL_NAMES.SUPPORT_HANDOFF) return isTrustedActor(actor) ? 'trusted_handoff' : 'trusted_actor_required';
    return 'restricted';
  }

  isToolVisible(tool, actor = {}) {
    const name = normalizeToolName(tool);
    if (PUBLIC_SAFE_TOOLS.has(name)) return true;
    if (name === AGENT_TOOL_NAMES.DRAFT_CAMPAIGN || name === AGENT_TOOL_NAMES.GENERATE_SHARE_KIT) return true;
    if (name === AGENT_TOOL_NAMES.EXPLAIN_WALLET || name === AGENT_TOOL_NAMES.SUPPORT_HANDOFF) return isTrustedActor(actor);
    return false;
  }

  assertToolAllowed(tool, input = {}, actor = {}) {
    const name = normalizeToolName(tool);
    if (!Object.values(AGENT_TOOL_NAMES).includes(name)) {
      throw new ValidationError('Unknown agent gateway tool.', { tool });
    }
    ...
    if (name === AGENT_TOOL_NAMES.SUPPORT_HANDOFF) {
      if (isTrustedActor(actor)) return name;
      throw new ForbiddenError('Support handoff requires a trusted gateway actor or user context.');
    }
    if (name === AGENT_TOOL_NAMES.DRAFT_CAMPAIGN || name === AGENT_TOOL_NAMES.GENERATE_SHARE_KIT) {
      if (input.persist === true && !isOperator(actor)) {
        throw new ForbiddenError('Persisting agent-generated campaign or share assets requires operator context.');
      }
      return name;
    }
  ...
  async auditTool(tool, input = {}, actor = {}, result = {}, success = true) {
    ...
    delete safeInput.token;
    return this.referralService.recordReferralEvent({
      event_type: success ? AGENT_GATEWAY_EVENT_TYPES.TOOL_EXECUTED : AGENT_GATEWAY_EVENT_TYPES.TOOL_FAILED,
      campaign_id: result?.campaign?.id || result?.campaign_id || null,
      code_id: result?.code?.id || result?.code_id || null,
      coupon_id: result?.coupon?.id || null,
      wallet_transaction_id: result?.wallet_transaction_id || null,
      subject_type: 'agent_gateway_tool',
      subject_id: tool,
      channel: input.channel || actor.surface || REFERRAL_CHANNELS.WEB,
      session_id: input.session_id || actor.session_id || null,
      metadata: {
        tool,
        success,
        input: safeInput,
        result_summary: result?.summary || result?.intent || result?.plain_language || null,
        surface: actor.surface || input.surface || null,
      },
    }, { ...actor, actor_type: ACTOR_TYPES.AGENT });
  }

  async executeTool({ tool, input = {}, context = {} } = {}, actor = {}) {
    const normalizedTool = this.assertToolAllowed(tool, input, actor);
    ...
    try {
      ...
      return {
        success: true,
        tool: normalizedTool,
        permission: this.describePermission(normalizedTool, actor),
        writes: WRITE_TOOLS.has(normalizedTool),
        audit_event_id: auditEvent?.id || null,
        result,
      };
    } catch (error) {
      await this.auditTool(normalizedTool, mergedInput, actor, { summary: error.message }, false).catch(() => null);
      throw error;
    }
  }
  ...
  async triage(input = {}, actor = {}) {
    ...
    const result = {
      intent,
      confidence: intent === 'general_triage' ? 0.52 : 0.82,
      suggested_tools: suggestedToolsForIntent(intent),
      required_fields: intent === 'referral_validation' ? ['code'] : intent === 'wallet_explanation' ? ['user_id'] : [],
      channel: input.channel || actor.surface || REFERRAL_CHANNELS.WEB,
      safe_response: 'I can help with referral codes, campaign status, share kits, wallet explanations, or handoff to CarUp support.',
    };
    await this.referralService.recordReferralEvent({
      event_type: AGENT_GATEWAY_EVENT_TYPES.TRIAGE_COMPLETED,
  ...
  async validateReferralCode(input = {}, actor = {}) {
    const result = await this.referralService.validateReferralCode({
      code: input.code,
      channel: input.channel || actor.surface || REFERRAL_CHANNELS.WEB,
      source: input.source || 'agent_gateway',
      session_id: input.session_id || actor.session_id || null,
      subject_type: input.subject_type || 'agent_gateway_session',
      subject_id: input.subject_id || input.session_id || actor.session_id || null,
      metadata: cleanObject({ ...input.metadata, surface: actor.surface || input.surface || null }),
    }, { ...actor, actor_type: ACTOR_TYPES.AGENT });
    return {
      ...result,
      next_actions: result.valid
        ? [AGENT_TOOL_NAMES.GENERATE_SHARE_KIT, AGENT_TOOL_NAMES.CHECK_CAMPAIGN_STATUS]
        : [AGENT_TOOL_NAMES.SUPPORT_HANDOFF],
    };
  }
  ...
  async draftCampaign(input = {}, actor = {}) {
    ...
    if (input.persist !== true) {
      return { persisted: false, draft, requires_human_review: true };
    }
  ...
  async generateShareKit(input = {}, actor = {}) {
    ...
    const validation = await this.referralService.validateReferralCode({
      code: input.code,
      channel: input.channel || actor.surface || REFERRAL_CHANNELS.WEB,
      source: 'agent_gateway_share_kit',
      session_id: input.session_id || actor.session_id || null,
    }, { ...actor, actor_type: ACTOR_TYPES.AGENT });
    if (!validation.valid) return { persisted: false, valid: false, validation };
    if (input.persist === true) {
      const shareAsset = await this.referralService.createShareAssets({
        code: validation.code.code,
        channel: input.channel || actor.surface || REFERRAL_CHANNELS.WEB,
        asset_type: input.asset_type || 'agent_share_kit',
        options: { ...this.shareOptions, ...cleanObject(input.options) },
      }, { ...actor, actor_type: ACTOR_TYPES.AGENT });
      return { persisted: true, valid: true, shareAsset, validation };
    }
    const payload = buildReferralShareAssets(validation.code, null, { ...this.shareOptions, ...cleanObject(input.options) });
    return { persisted: false, valid: true, share_kit: payload, validation };
  }
  ...
  async explainWallet(input = {}, actor = {}) {
    ...
    if (userId !== actor.actor_user_id && !isAdmin(actor)) {
      throw new ForbiddenError('Agent gateway cannot explain another user wallet without admin context.');
    }
  ...
  async openSupportHandoff(input = {}, actor = {}) {
    ...
    const event = await this.referralService.recordReferralEvent({
      event_type: AGENT_GATEWAY_EVENT_TYPES.SUPPORT_HANDOFF_OPENED,
      code_id: input.code_id || null,
      campaign_id: input.campaign_id || null,
      subject_type: input.subject_type || 'support_handoff',
      subject_id: input.subject_id || input.user_id || actor.actor_user_id || input.session_id || null,
      channel: input.channel || actor.surface || REFERRAL_CHANNELS.WEB,
      session_id: input.session_id || actor.session_id || null,
      metadata: {
        issue_type: issueType,
        summary: input.summary || input.message || 'Agent gateway support handoff requested.',
    ...
    return {
      ...
      issue_type: issueType,
      summary: input.summary || input.message || null,
    };
  }

  async checkCampaignStatus(input = {}) {
    const campaigns = await this.referralService.listCampaigns({
      tenant_id: input.tenant_id || undefined,
      status: input.status || undefined,
      campaign_type: input.campaign_type || undefined,
      priority_scope: input.priority_scope || undefined,
    });
    return {
      ...
      campaigns: campaigns.map((campaign) => ({
        ...
        priority_scope: campaign.priority_scope,
        status: campaign.status,
        route_origin: campaign.route_origin || null,
        route_destination: campaign.route_destination || null,
  ...
  async draftSocialCopy(input = {}, actor = {}) {
    let validation = null;
    if (input.code) validation = await this.validateReferralCode(input, actor);
    return buildSocialCopy(input, validation);
  }
}
