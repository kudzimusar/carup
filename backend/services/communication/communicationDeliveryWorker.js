...
export class CommunicationDeliveryWorker {
  ...
  async processDueNotifications({ limit = 10 } = {}) {
    const rows = (await this.repository.list('notification_queue', {}, { order: { column: 'scheduled_at', ascending: true }, limit: 200 }))
    ...
    for (const notification of rows) {
      results.push(await this.deliverNotification(notification));
    }
    return results;
  }
