...
- **Operator-blocked (cannot run from here):** applying migrations to staging Supabase; deploying head `4a50a33` frontend/backend; live on-device WhatsApp + Telegram inbound→reply→receipt UAT (real `wamid`/`telegram_bot_api` ids); a controlled failed→dead-letter recovery; desktop/tablet/mobile screenshots. No production resource was modified.
