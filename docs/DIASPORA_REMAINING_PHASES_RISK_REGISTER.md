...
| SEC-DB-2 | MED (OUT-OF-SCOPE, noted) | The static security scanner (`diaspora-staging-apply-verify.mjs`) flags foundation migrations `013_diaspora_trade_schema` (authenticated write grant) and `20260611061849_phase1b_foundation` (9× `FOR ALL` authenticated policies on workbook/stock/drive tables) with the same class of pattern. | OPEN (deferred) | A | Explicitly **out of the Phase 8/9 hardening scope** (directive: do not expand into unrelated repo-wide security work). Recorded for a dedicated follow-up review; the Phase 3-7 stock/workbook invariants are additionally guarded by the H1 append-only ledger RPC + `20260619201406` containment. Adjudicate before public production. |
