...
| ST-4 | LOW | UI-9 fail-safe completeness items surfaced by the adversarial review (NOT bypasses — the backend stays authoritative and all are fail-closed): (a) the `request-release` available-actions projection mirrors only the reviewer `MARK_ARRIVED` edge, so it under-represents the participant milestone-staging path the route also accepts (projection is STRICTER than the route — never over-permits); (b) no server-redacted dispute-evidence READ endpoint is wired (`getSafeTradeDisputes` returns no evidence join and no `GET …/evidence` hook exists), so the dispute evidence timeline renders empty in production and the client `visibleEvidence` filter is exercised only against mocked data. | OPEN | A | Server-side redaction remains the real privacy guarantee (proven by listEvidence tests); the client filter is defense-in-depth and now fails CLOSED on unknown visibility. Before EB-4: (a) extend the projection to also surface the participant milestone path OR rename that capability; (b) add a redacting `GET /safetrade/disputes/:id/evidence` route + hook so the timeline is live. |
