...
| ID | Sev | Risk | Status | Owner | Mitigation / boundary |
| --- | --- | --- | --- | --- | --- |
| DOM-1 | HIGH | SafeTrade releasing money/escrow on a frontend click; auto compliance/shipment/delivery/reputation. | OPEN | E | State machine + release policy engine; reviewer approval for high-risk; sandbox-only provider; non-negotiables enforced + tested (§50). |
