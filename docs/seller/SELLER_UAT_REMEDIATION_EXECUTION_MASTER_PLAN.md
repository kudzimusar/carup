...
- [~] **F18. Persist structured accident disclosure through guest draft → registration/login → authenticated server draft → refresh/resume.**
  ...
  - **Evidence:** commits `b173d9e0` (backend) + `e982a0a4` (frontend). Closed vocabulary yes/no_known_accident_history/unknown in `backend/services/seller/vehicleHistoryDisclosures.js` + `web/src/lib/vehicleHistoryDisclosures.ts`; structured events (approx_date, mileage, damage_area, severity, insurer_involved, police_report_state, repair_state, repairer — allow-list projected, max 10, 200-char cap). Stored in `vehicles.seller_accident_disclosure` (migration `20260831150000`, state CHECK, no default — NULL = "not recorded"). Wired: GuestSell stage 1 UI → sessionStorage draft (never-repair parser) → auth handoff hydration → revision-safe server autosave (answered-only keys; receipt deep-equality gate) → server-draft resume parse → POST /vehicles/add (fresh + re-list rows, claim-source stamped, 503 fail-closed on old schema). Tests: `backend/tests/seller-history-disclosures.test.js` 11/11; `VehicleHistoryDisclosuresSection.test.tsx` 8/8; self-updating continuity guards (`guestSellDraft.continuity.test.ts`, `seller-data-contract-completeness.test.js`) green with the new fields; seller web battery 59/59, backend battery 62/62.
  - **Gap:** unit/contract-proven at every layer, but the full continuity path has no EXECUTED deployed end-to-end run yet (that proof lands with S55-style staging assertions); attribution to a *distinct* buyer-facing surface (K17) not yet built.
...
- [~] **F19. Persist structured current-insurance disclosure through the same continuity path.**
  ...
  - **Evidence:** same commits/rails as F18; column `vehicles.seller_insurance_disclosure` with insured/not_insured/unknown CHECK; optional insurer_name kept only for an "insured" answer; nothing in the write path touches `insurance_records` (seller_* column class only, same authority separation as seller_description). UI copy states the separation ("CarUp shows confirmed cover separately when an insurer record exists").
  - **Gap:** as F18 (no executed deployed E2E yet; K18 governed-vs-stated rendering not yet built).
...
- [~] **F20. Persist structured vehicle finance/lease/lender-interest disclosure through the same continuity path.**
  ...
  - **Evidence:** same commits/rails as F18; column `vehicles.seller_finance_disclosure` with none_known/active/settlement_pending/cleared/unknown + finance_type (bank_loan/vehicle_finance/lease/hire_purchase/secured_lien/other) CHECKs; **private banking terms are refused at the API (400) AND banned by a migration CHECK** (balance/APR/monthly payment/account-loan-contract identifiers/repayment history/credit data — M17/INV-18), with the module and migration ban lists compared verbatim in `seller-history-disclosures.test.js` so they cannot drift. No code path reads or writes `finance_applications`; the UI privacy note states no such data is collected and that active finance does not block listing (settlement handled at transfer).
  - **Gap:** as F18; the governed vehicle-obligation/encumbrance authority (lender-attested truth this statement is later compared against, M16/R23-R25) is a separate upcoming slice.
...
- [~] **K17. Shared Buyer Preview / Marketplace Vehicle Detail contains a first-class Accident, damage & repair history section.**
  ...
  - **Evidence:** commit `b0a61c36`. `web/src/components/vehicle/VehicleHistoryObligationsSections.tsx` renders §14 item 9 (`detail-accident-history-section`) inside `VehicleDetail.tsx` — the one shared buyer component — splitting the Seller declaration (badged **Seller-stated**, with structured events: date, mileage, damage area, severity, insurer involvement, police report, repair state, repairer) from a separate governed-state paragraph pointing at the Vehicle History/Evidence sections. Unanswered renders `history-accident-not-recorded` ("Not recorded … absence of an answer is not a clean-history claim"). Data via the single `toVehicleHistoryDisclosures` projection on both the passport and marketplace detail payloads. Tests: `VehicleHistoryObligationsSections.test.tsx` 6/6 (incl. an assertion that no affirmative "no accident"/"clean history" string can render); `seller-history-disclosure-projection.test.js` 5/5; VehicleDetail suites 128/128.
  - **Gap:** the governed half currently states its honest "no connected source / see history & evidence" position; rendering ACTUAL insurer/police/garage-sourced accident evidence with per-source provenance inside this section is the G5/L24 slice and is not yet built.
...
- [~] **K18. Shared buyer presentation contains a current Insurance state/history section.**
  ...
  - **Evidence:** same commit/component. `detail-insurance-section` (§14 item 10) shows the Seller declaration badged Seller-stated (insurer name suffixed "(seller-stated)") above a separate `history-insurance-governed-state` line: *"Insurer-confirmed cover: no connected insurer source for this vehicle. A seller saying 'insured' never becomes provider confirmation."* Nothing in the write or read path touches `insurance_records`, so the seller statement cannot masquerade as provider truth (M15 posture).
  - **Gap:** reading the governed `insurance_records` / insurer-provider authority into this section (so a real policy displaces the "no connected source" line) is not yet implemented — currently truthful-but-empty on the governed side.
...
- [~] **K19. Shared buyer presentation contains a Finance, lease & title-obligations section.**
  ...
  - **Evidence:** same commit/component. `detail-finance-obligations-section` (§14 item 11) publishes only public-safe state — finance state, finance type, seller-stated lender name — and for `active`/`settlement_pending` renders `history-finance-transfer-condition`: *"does not prevent viewing or inquiring about this listing. Settlement or lender clearance is required before ownership transfer can complete"* (R23 posture). Private terms are excluded three times over: refused by the API (400), banned by the migration CHECK, and stripped by the projection allow-list — with a test asserting a balance/APR passed directly as props still cannot render.
  - **Gap:** valuation-at-origination/date/source and lender-confirmed clearance state require the governed vehicle-obligation/encumbrance authority (M16/R24–R26), which is the next slice; the section shows "no connected lender source" until then.
...
- [~] **K20. Buyer Preview and public Marketplace use the same components/data semantics for K17–K19; preview mode does not invent stronger evidence than public mode.**
  - **Evidence:** structurally guaranteed rather than asserted per-mode: `VehicleDetail.tsx` renders exactly one `<VehicleHistoryObligationsSections>` outside every `isSellerPreview` branch, fed by one projection (`toVehicleHistoryDisclosures`) that takes no audience/mode argument — so preview and public cannot diverge without deleting the shared call site.
  - **Gap:** no executed side-by-side preview-vs-public assertion yet (belongs with S57); the guarantee is currently by construction plus code reading.
...
- [~] **R23. An active vehicle finance/lease/lender interest can coexist with a public listing, but the transaction/title state explicitly says settlement or lender clearance is required where applicable.**
  ...
  - ~~Gap: the buyer-facing UI does not yet RENDER the governed `finance_obligation` block.~~ **CLOSED @f37818c8** — `VehicleHistoryObligationsSections.tsx` now renders the governed block with state-specific copy, and keeps three DISTINCT honest answers: recorded obligations; "no lender record is held for this vehicle in the sources CarUp can read — that is not a guarantee that none exists" when a governed source exists but holds nothing; and "no connected lender source" when there is no channel at all. The middle state is the one that matters: an available-but-empty source is never published as "unencumbered".
...
- [x] **R27. Insurance claim → accident event → repair evidence can remain visible after listing unpublish/sold; commerce lifecycle never deletes the durable history.**
  - **CLOSED (V16 convergence).** Source/test resolvable — NOT provider-gated, unlike R23/R24/R28. The publicly visible chain comes from `vehicle_evidence` class mapping (damage_photo→accident, repair_photo→repair, insurance_document→insurance), from `vehicles.seller_*_disclosure` and from `partsentry_logs`; `insurance_records` is read privileged-only and reports `source_states.insurance_registry = 'unavailable'` to a public audience, so no insurer feed is required to close this.
  - **Evidence:** `backend/tests/r27-durable-history-survives-commerce.test.js` — 9/9. It proves, hermetically: publish and unpublish write `publication_status` ALONE and mark-sold writes `status` ALONE (parsed from the real handlers); the vehicle lifecycle routes issue NO delete at all, so the `ON DELETE CASCADE` foreign keys can never fire from a commerce action; `buildVehiclePassport` and the `/passport` route consult neither `isPublicVehicleStatus` nor `isPubliclyVisiblePublication`, so the durable record survives withdrawal while the gallery gate stays scoped to the gallery; and the history projection is byte-identical across published → unpublished → sold, keeping `authority: 'seller_stated'`.
  - **Anti-vacuity:** the same file proves commerce DOES hide the listing — `isPublicVehicleStatus('Sold')` and `isPubliclyVisiblePublication('publishable')` are both false and `/api/vehicles/:vin/details` 404s — so "history survives the gate" is a claim about a gate that demonstrably exists. It also proves an UNANSWERED topic still reads "not recorded" after sold, never "no accidents".
...
- [~] **V19. Resumed server draft saves by reusing its Passport; missing identity is publication-only and fill-once.**
  - Owner UAT 2026-09-01: the Serena was visible in My Garage, but explicit Save returned **409 EXISTING_PASSPORT_CONFIRM_REQUIRED** because the restored Seller page still POSTed `/api/vehicles/add` without setting `reuse_existing_passport`; a route-level **503** during deployment made the symptom look like another lost save.
  - Root cause: server-draft hydration set `existingPassportConfirmed=true` and Seller authority to recognized, but submit derived reuse only from the asynchronous Passport lookup state. A restored draft therefore depended on a second network verdict even though `/api/vehicles/me` had already established governed scope.
  - Remediation: a server-restored draft whose VIN matches the scoped server vehicle explicitly reuses the existing Passport; the 409 is surfaced truthfully rather than as generic “Failed to save”. On Passport reuse, Seller may fill only identity fields that are currently NULL (plate/temp permit/chassis/engine); any attempted overwrite returns `SELLER_IDENTITY_CONFLICT_REVIEW_REQUIRED` and leaves canonical identity unchanged.
  - Product copy: “Missing identity fields” is explicitly publication readiness, not a draft-save failure.
  - Evidence required for `[x]`: exact-head CI + Seller lifecycle green; owner saves `GFC27-027051` from the resumed URL without a duplicate-VIN 409, sees it remain in My Garage/My Listings, and can add a previously missing plate/temporary permit without overwriting recorded identity.
