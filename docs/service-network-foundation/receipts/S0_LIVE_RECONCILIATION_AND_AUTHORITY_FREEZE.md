...
- **Communications:** ~~`business_workflow='service'` added~~ — **AMENDED at S3 on evidence**: the canonical stakeholder contract already declares a `garage` workflow with roles exactly `['vehicle_owner','garage']`, `thread_type 'general'`, and an `emailStakeholderMatrix` row whose declared identity source is literally "work order participant -> channel_identities" — it was defined for this interaction and has no producer. Adding a near-duplicate `service` workflow would create two competing conversation keys for one interaction, so Service Case conversations REUSE the `garage` workflow and are distinguished by subject (`subject_type='service_case'`, `subject_id=<case id>`), which the deterministic thread key (`workflow|tenant|subject_type|subject_id`) already makes unambiguous. No new workflow, contract or matrix row; `thread_type` stays `'general'`; `subject_type='service_case'` frozen (single spelling — thread dedupe forks otherwise); notifications channel-scoped **in_app/transactional-first** (recipient-address enrichment gap is real; email/WhatsApp/push after rebase onto #194's recipientResolution); WhatsApp requires a Meta-approved template — none exists, none sent (plan §35); **REAL-SEND HAZARD** honored: staging pg_cron drains with real adapters — tests never enqueue against staging Postgres, `COMMUNICATION_REAL_ADAPTERS` stays unset. Communications failure never rolls back a Service Case (§15.5 — recoverable receipt pattern).
