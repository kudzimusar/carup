...
| 19 | `20260726120000_diaspora_phase8_9_10_client_grant_hardening.sql` | SEC-DB-3 (compensating) | Close the Supabase default-privilege grant gap on the 19 Phase 8/9/10 tables: explicit `REVOKE ALL FROM anon`, revoke `authenticated` writes (INSERT/UPDATE/DELETE/TRUNCATE/REFERENCES/TRIGGER), explicit `GRANT SELECT` to `authenticated` (every table has an authenticated SELECT policy), `GRANT ALL` to `service_role`. ACL statements only — no data/columns/constraints/indexes/policies/functions/flags. Every table named explicitly; idempotent; one transaction. | grants on 19 tables (6 Phase 8, 6 Phase 9, 7 Phase 10) | grants Y (anon=NONE; authenticated=SELECT-only; service_role full) | N (tightening only — reversing re-opens client grants; restore-from-backup) | `6674706778b5` | PENDING (workflow dispatch) | PENDING (EB-5 gate E) |
