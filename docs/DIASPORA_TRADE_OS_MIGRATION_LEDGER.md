...
| 17 | `20260718100000_diaspora_foundation_mutation_boundary_hardening.sql` | SEC-DB-2 (compensating) | Enforce server-authoritative mutation boundaries on the 27 foundation lifecycle/data tables that shipped `FOR ALL` policies + authenticated INSERT/UPDATE grants (013/phase1b). REVOKE anon + authenticated write on every table; `FOR ALL`→`FOR SELECT` (same read predicate); helper hardening (pin search_path, revoke anon, actor-spoofing guard on is_tenant_member/is_platform_admin). No table recreated. | alters grants/policies on 27 tables; recreates 2 authz helpers | RLS Y (all SELECT-only for authenticated; service_role writes) · grants Y (no authenticated write; anon none) · helpers search_path pinned + anon revoked + self-only guard | N (tightening only — reversing re-opens writes; restore-from-backup) | `f81c52de296b` | NOT YET APPLIED | NOT APPLIED (EB-5) |
