...
## Final predictive closure (candidate `26cfc1ee` → F head) — bounded to three predicted issues

- [x] F1 **REPRODUCED end-to-end** — a version number is not proof of authorship. A pre-contract row containing `{v:1, checksum_source:'server_inline'}` was believed, and a genuine INLINE upload of a different object was handed the forged row. Closed with an **HMAC-signed** provenance assertion **bound to the row** (source + checksum + vin + uploader), keyed from the existing required `JWT_SECRET` under a domain-separated label — no new provider, no new secret, no migration. Missing key material reads as UNTRUSTED (conservative), never fail-open. 2 mutations red
- [x] F2 **REPRODUCED** — `file_url` and `file_path` are accepted independently; identity read `file_path || file_url`, so holding the path constant while changing the URL silently discarded the second document. A URL locator now decides identity, and contradictory dual locators are refused with 400 BEFORE any write. Trusted-storage-URL + matching path still works; signed reissue still dedupes; inline unaffected. 1 mutation red
- [x] F3 **REPRODUCED** — register and behaviour were disconnected. Register extended to 16 entries (added `GET /api/vehicles/:vin/seller-authority`, which consumes the governed primitive and reports Seller state) and each entry now NAMES its behavioural scenario; the suite fails if a surface has no scenario or a scenario has no surface. Source anchors demoted to rename detection only, stated in-code. 2 mutations red
- [x] F3b Behavioural route proofs added for the two P3 surfaces plus the seller-authority state read; `hasExistingSellerRelationship` proved directly as a service primitive
- [x] F3c **MEASURED LIMIT recorded, not glossed:** `/api/vehicles/add` is defended twice — for the adversary the K3/L2 creation-eligibility gate refuses first, so restoring raw membership in the reuse clause ALONE is not observable there. The clause is proven load-bearing in the observable direction: disabling it turns the positive control red
- [x] F3d A first version of the `/vehicles/add` scenario asserted only `!== 201` and stayed GREEN under the acceptance mutation because both actors were failing validation. Replaced with a complete payload and discriminating assertions
