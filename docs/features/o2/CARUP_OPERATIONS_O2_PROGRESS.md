...
- [~] X5A Stakeholder Workbook Catalogue + AI Intake Workspace — **IN PROGRESS 2026-09-04** (docs-first); plan `CARUP_OPERATIONS_O2_X5A_STAKEHOLDER_WORKBOOK_AI_INTAKE_PLAN.md`; manual `CARUP_OPERATIONS_O2_STAKEHOLDER_WORKBOOK_CATALOGUE.md`
- [ ] X5A.1 Stage A documentation gate — plan + catalogue manual + 6 canonical doc updates; every stakeholder dispositioned; field registry documented; docs-only commit BEFORE code
- [ ] X5A.2 Workbook Field Registry (code) — versioned; powers templates/labels/help/validation/vocab/stakeholders/worksheets/import-export flags/AI metadata; authority-only fields not importable
- [ ] X5A.3 Engine reconciliation — xlsx service accepts template objects (diaspora strings byte-equivalent, xlsx suite green unmodified); X5 mapping service generalized to registry columns
- [ ] X5A.4 Template Catalogue API — server-derived available/unavailable + reasons; forged request-body eligibility refused
- [ ] X5A.5 Seller/Dealer vehicle workbook — normalized sheets covering the documented current user-enterable Seller contract; import chain (inspect→map→confirm→validate→dry-run→confirm→execute→receipt) through canonical services only
- [ ] X5A.6 Importer/Exporter + other stakeholders — diaspora contracts reused; unsafe/deferred templates honestly unavailable
- [ ] X5A.7 CarUp AI Workbook Assistant — visible UI + endpoints; deterministic first; proposals visually distinct; authority matrix enforced; failure degrades to manual
- [ ] X5A.8 Export + Recent Imports — server-sourced scoped redacted export; existing batch/receipt store reused and caller-scoped
- [ ] X5A.9 Tests — exposure per stakeholder; registry completeness drift-fails-loudly; no-re-entry proof; template versioning; security/authority refusals; full regression (X1–X5, P1-C, diaspora/seller/dealer, full backend, full web, tsc, lint 0/0)
- [ ] X5A.10 Receipt `CARUP_OPERATIONS_O2_X5A_STAKEHOLDER_WORKBOOK_AI_INTAKE_RECEIPT.md` + live doc updates throughout; STOP before X6
