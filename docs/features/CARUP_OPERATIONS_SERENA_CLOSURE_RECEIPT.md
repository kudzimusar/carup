...
| 19–21 | Desktop / tablet / mobile | **12/12 passed** on all three Chromium profiles — final certification run **33677541343 @ `a9339b94`, conclusion SUCCESS** |
...
| 31 | Broader CI matrix | Classified below — no unexplained gate. NOTE: `CI` triggers only on `pull_request: branches:[main]`, so the (correct) retarget removed it from this PR; its gate was reproduced locally at the frozen SHA — web typecheck PASS, backend suite = baseline only, **zero net-new lint findings** (identical `any` counts vs the parent lane in every file touched). CI will gate these changes when lanes #205/#194 target main. |
...
| 33 | Credentials / test data | Kingstone's credential captured to the runner and restored (`always()` step, asserts exactly one row); hash well-formed scrypt. No secret committed. No backup table, no stray privileged account, no orphan authority rows, no duplicate or synthetic Serena. Seven guest inquiries accumulated across certification rounds (one per desktop mutation pass, by design — each proves the path); they are genuine UAT evidence and were kept. `trust_audit_events` stayed at **14 across every re-run**, which is the idempotency proof: the state-aware journey genuinely skipped re-deciding rather than re-writing governed facts |
