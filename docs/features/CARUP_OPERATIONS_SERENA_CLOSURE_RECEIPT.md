...
| 39 | **Merge order — CORRECTED** | **#206 → #205 → #194.** The earlier receipt recorded `#194 → #205 → #206`, which is inverted for a stacked chain: merging #194 first promotes `integration/vehicle-passport-v16-cert` to `main` **without** this slice, leaving #205 and #206 still stacked underneath and requiring a fresh integration→main PR afterwards. Landing this work means merging the deepest PR first. Verified topology: `main`(`ba208963`) ← `integration/…`(`f180c47d`) ← `fix/zimbabwe-…`(`569e4f14`) ← this branch. Parallel lane #200 (Seller UX convergence) also targets #194 and touches My Garage / My Listings / Sell router; it must be reconciled on that lane, not here |
| 39a | **What the certification does and does not cover** | `fix/zimbabwe-seller-reality-comms-hardening`'s tip (`569e4f14`) is **exactly** this branch's merge-base, so **step 1 is a clean fast-forward** — the certified tree and the merged tree are the same tree, and this receipt fully covers it. Steps 2 and 3 are a different matter and are **not** covered: `integration/…` is **not** an ancestor of `fix/…`, so #205 combines this slice with integration work that no run here exercised. #205's own gates must run **after** step 1, on the combined result. Do not treat this receipt as certification of that combination |
