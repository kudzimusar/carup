...
| 2026-09-03 | Publication re-materializes the canonical Trust position | Publication is the moment CarUp asserts a public position, so it is the correct barrier for re-deriving the stamp a buyer reads. Without it a listing goes public carrying a conclusion older than its own facts — GFC27-027051 published "registration stage has not been established" beside a claim block reporting that stage as recorded. Best-effort and after the state change, so it can never refuse a legitimate publication | Editing the Trust row directly (forbidden); re-verifying evidence to force a refresh (rewrites reviewer attribution and fabricates a review); the seller re-saving through the reuse path (unpublishes the live listing); changing the score display (hides the contradiction rather than resolving it) |
| 2026-09-03 | Evidence visibility is decided server-side; narrowing is free, widening needs the review capability | The request body previously won outright over the server default, so an uploader chose whether their own source document was published. The Serena's Tanzania T1 reached buyers that way, from a provenance chain whose only event is the owner's own upload — seller self-certification of publication, which §3.11/G7 forbid. A client-side default is not a control | Rejecting the upload outright (punishes the seller for a client's choice and loses a legitimate artifact); trusting the corrected web client alone (a stale or non-web client still decides); a role check in the UI only |
| 2026-09-03 | A refused widening is clamped and RECORDED on the row, not silently dropped | Review needs to see that an uploader asked for a wider audience than their authority allows, and a stale client that keeps asking should be visible rather than invisible | Silent clamping; a 400 that loses the artifact |
| 2026-09-03 | Visibility corrections travel through the EXISTING classification-correction primitive | Correcting what a record IS and correcting how widely it is published are the same governed act over the same row, so they share one reason, one audit event and one history entry. There was previously no post-upload writer for visibility_level anywhere in the backend, so a wrongly published document could only be withdrawn by a service-role write with no actor, no reason and no audit | A second endpoint; a bulk visibility tool; leaving it to direct SQL |
| 2026-09-03 | Corrections are offered on already-verified evidence; Verify/Reject stay pending-only | A mis-classified or wrongly published document is almost always discovered AFTER review. Gating the correction editor on pending made the capability dead by construction for exactly the rows that need it — the real T1 is verified and published, and was unreachable from the workspace | Keeping the pending gate and correcting by SQL; allowing re-verification (rewrites verified_by/verified_at and destroys attribution) |
| 2026-09-03 | The published synthetic Golden vehicle in staging is attributed, not remediated | JTDKARFP0H3000731 was created 2026-06-17, eleven weeks before this slice; unpublishing another lane's fixture from here could red their gates. Detection plus attribution is the honest action; the decision belongs to that lane's owner | Silently unpublishing it; ignoring it entirely |
