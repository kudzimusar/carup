...
**Status:** M0–M7 CLOSED — CERTIFIED MERGE CANDIDATE (UNMERGED). **M8 (architecture extraction) COMPLETE — documentation only, no product code changed, certification unaffected.** M0–M7 executed, hardened, and taken through a residual closure pass. The real Serena was reviewed under governed authority and PUBLISHED BY KINGSTONE. The frozen candidate is **`f25ea5c6`** (superseding `a9339b94`, which the residual pass invalidated by changing product code): 15/15 on desktop/tablet/mobile and **8/8 gates green at that SHA**, including the Golden Seller lifecycle that was previously deferred. Closure receipt: docs/features/CARUP_OPERATIONS_SERENA_CLOSURE_RECEIPT.md. PR #206 retargeted to its true parent lane (its real slice diff, not 760 files of inherited ancestry). The two previously-open items — the stale Trust sentence and the T1 published as `public_safe` — are both CLOSED, each through a governed product path rather than a data edit.
...
- [x] M0.20 Visibility + verification — all pending; visibility `private` except Tanzania T1 = `public_safe`. Leak check: all five are documentEvidenceTypes → stored in the private `ocr-documents` bucket → `toPublicEvidence` withholds file_url regardless of label; nothing currently leaks (also all pending, so no public path applies). **RESOLVED in the residual pass:** the T1 label was not merely cosmetic — once the row was verified it became the ONLY row passing the guest filter, so it alone was published to buyers as a governed fact, and its provenance chain held exactly one event (the owner's own upload). That is seller self-certification of publication. The upload route now decides visibility server-side, the governed correction primitive can withdraw a wrongly published document, and the T1 is `restricted`. See closure receipt 35a′.
...
| Old eligibility doc claims 17-char VIN only | docs/CARUP_REAL_LISTING_ELIGIBILITY_CONTRACT.md §2 still says 17-char; Serena (12-char chassis-as-VIN) exists as a real listing row; manual §3.5 says code recognizes 12–17 import identifiers | Docs stale vs code | **DONE (residual pass):** both drifted rows corrected against the helper, with a dated correction note naming GFC27-027051 as the live counter-example |
| Seven-document pack in Evidence Vault | Only 5 of 7 uploaded (payment receipt + identity pending); canonical classes all correct, legacy types all wrong; T1 labeled public_safe | Partially different | M7 review handled the classification decisions; **the T1 visibility is RESOLVED in the residual pass** (withdrawn to `restricted` through a governed, audited correction); payment receipt upload remains Kingstone's/UAT action; identity goes through the identity workflow, never the vehicle vault |
...
- [x] M7.19 Canonical Trust state recorded. — Canonical Trust materialized after evidence verification — 46 / moderate. **Corrected in the residual pass: 60 / moderate.** The 46 was stale, not wrong-by-rule: the stamp predated the seller's registration-stage statement and, carrying the current calculation version, classified as FRESH so no read path would recompute it. Publication now re-materializes the position, and the seller's own republish healed it. The score followed the facts; the facts were not moved to follow the score.
...
- [x] M7.41 Seller Golden lifecycle — **CLEARED (residual pass): run 33698702769 @ `f25ea5c6`, SUCCESS.** The deferral below was a concurrency hazard, not an unresolvable one — the two workflows overlap on exactly one identity and each rotates-then-uses it inside its own run, so sequential execution is self-contained. Proven the hard way: dispatching it while the Serena run held `uat.reviewer` produced exactly the predicted collision, so it was cancelled and re-run afterwards. Original reasoning retained:  The certified Golden gate (`seller-exact-head-staging-uat.yml`) triggers on PRs into `integration/vehicle-passport-v16-cert` and rotates the SAME `uat.reviewer@carup-staging.test` identity this slice's certification rotates. Running both against one staging project concurrently invalidates whichever credential rotates second (the recorded concurrent-session hazard). Its refusal contract was updated for M3 (`seller_authority`), and its spec runs unchanged on the parent lane. It must run on lane #205/#194 before their merge; this slice does not silently claim it.
...
- [x] M8.1 Compare Vehicle Operations workflow with Communications workflow. — 19-dimension comparison in the ADR §4. Genuinely horizontal: state/who-must-act, audit, resolution, reopen, permissions, private evidence, canonical-authority separation. Communications-specific: teams, SLA, dead-letter/delivery recovery, public customer messaging. Vehicle-Operations-specific: the work item IS the vehicle (no thread identity needed), and external-authority wait states. **Headline finding: 'who must act next' was independently invented THREE times** — Communications `awaiting_ai/human/user`, Identity `WORKFLOW_PHASE`, Vehicle Ops `who_must_act` — and only Vehicle Ops has `external_authority`.
- [x] M8.2 Decide whether assignment is common enough to extract. — **DEFER.** Only ONE domain has a working assignment model (Communications: `assigned_admin_id` + `assigned_team` + routing + inbox facets); the second (marketplace inquiries `assigned_operator`) is one column and one endpoint. `review_tasks.assigned_to` is a DEAD column — `governanceService` never writes it. Vehicle Operations certified without wanting assignment at all. What IS proven is drift: five column names for one concept, so a naming contract is adopted now without a table. Re-open at O4.
- [x] M8.3 Decide whether SLA is common enough to extract. — **REJECT a horizontal SLA framework.** Exactly ONE domain has an SLA; every other `sla` hit outside `services/communication/` is the substring in *translate*. Others have bare deadlines (`review_tasks.due_at`, `disputes.response_deadline`) with no clock. Adopted instead: **an SLA clock may run only while `who_must_act = carup_review`** — a clock running while a seller, lender or ZIMRA holds the work publishes false blame against CarUp. `computeSlaState` is already a pure zero-DB function, so extraction is trivial whenever a second real consumer appears; moving it today would create a shared module with one caller.
- [x] M8.4 Decide whether generic operations_cases is justified. — **REJECT** the stateful table; **DEFER** a read-only Work Index. Decisive evidence: `review_tasks` ALREADY IS a generic operations-case table (`task_type` across 9 cross-domain types, polymorphic `target_type`/`target_id`, `assigned_to`, `priority`, `due_at`, `decision_id`, `metadata`) and after months has ONE writer. Building `operations_cases` would mean a second generic case table beside an existing unused one, duplicating canonical domain status in `fraud_cases`/`verification_sessions`/`vehicle_seller_authority`/`disputes`/`message_threads`. Its central risk — a derived record drifting from canonical truth — is exactly the Serena's stale Trust stamp, institutionalised across every domain at once.
- [x] M8.5 Decide whether persistent operations memberships/capabilities are justified. — **DEFER.** The static `platformRole`/`baseRole` map is sufficient exactly while every capability is held by a platform-level role as a block, which is true today. It breaks at the first specialist who must hold ONE capability without full admin (mechanic assessor, insurance reviewer, KYC specialist) — O2/O5/O7 work. Full design constraints recorded in ADR §8 (additive-to-static so a grant bug fails closed, platform-only grant authority, mandatory expiry on delegation, break-glass as a short-expiry grant not a bypass flag). Tenant `organization_roles`/`organization_permissions` must NEVER grant `operations.*`.
- [x] M8.6 Decide whether Seller Authority needs dedicated table/service beyond current implementation. — **KEEP DOMAIN-SPECIFIC; no structural change needed.** It is the most evolved governed-decision record in the codebase (the only one with BOTH `policy_version` and `basis`) and is the reference implementation for the extracted contract. Expiry not required; revocation sufficient; `UNIQUE(vin, seller_user_id)` is the right grain. Two real gaps recorded, neither urgent: `disputed` has no FK to `disputes`, and a completed ownership transfer should revoke the prior seller's authority (integration point for O2). Generic 'entity authority' REJECTED — one domain uses it.
- [x] M8.7 Record reusable pattern ADR/decision. — `docs/architecture/CARUP_OPERATIONS_CONTROL_PLANE_M8_REUSABLE_OPERATIONS_PATTERN_ADR.md`. Approves exactly three things, all contracts rather than tables so none can threaten domain truth ownership: the `who_must_act` vocabulary, the governed decision-record contract, and an assignment naming contract.
- [x] M8.8 Update canonical manual current-state section. — Manual §23A 'Post-Serena Proven Architecture' added and marked CURRENT; §§24–25 retained as history with an explicit precedence note.
- [x] M8.9 Update future O2–O10 sequencing. — Resequenced to **O2 → O3 → O4 → O10 → O6 → O7 → O8 → O5 → O9** with a dependency graph (ADR §16). Two corrections to the original order: O5 Service Network is a domain BUILD not an operations slice (`workOrdersRoutes.js` is 91 lines and `mechanic_work_orders` has no operations semantics), so it moves late; and O4 must follow O2/O3 because only then do assignment and SLA have the second consumers needed to decide their deferrals with evidence instead of guesswork.
