...
import {
  allowedRolesValue, buildRolloutPatch, formFromRow, parseRolloutPercentage, roleSummary,
  type EditForm, type RolesMode,
} from './featureGovernanceConsole.helpers'
import type { UserRole } from '@shared/types'

const ENVIRONMENTS = ['development', 'staging', 'production'] as const

/** Shape of the audit-history rows we render (the API returns `unknown[]`). */
interface AuditItem {
  event_type?: string
  created_at?: string
}
...
export default function FeatureGovernanceConsole() {
  ...
  const [audit, setAudit] = useState<{ loading: boolean; error: boolean; items: AuditItem[] }>({ loading: false, error: false, items: [] })
  ...
  // Behaviour is unchanged: the fetch still runs on mount and whenever `load`
  ...
  const openDetail = (row: AdminFeatureRow) => {
    ...
    api.getAudit(row.id)
      .then(res => setAudit({ loading: false, error: false, items: (res.audit || []) as AuditItem[] }))
  ...
  const doSave = async () => {
    ...
    try {
      ...
      if (/version_conflict/i.test(String(message))) {
        setConflict(true)
        setConfirm(null)
        toast.error('Version conflict — refresh and re-apply your change.')
      } else {
        toast.error(`Save failed: ${String(message || 'error')}`)
      }
    } finally {
      setSaving(false)
    }
  ...
  const doReset = async () => {
    ...
    try {
      ...
      toast.error(`Reset failed: ${String((err as { message?: string })?.message || 'error')}`)
    } finally {
      setSaving(false)
    }
  }
                ...
                <div className="text-sm">
                  ...
                  {!audit.loading && audit.items.length > 0 && (
                    <ul className="text-xs space-y-1 max-h-32 overflow-y-auto" data-testid="fg-audit-list">
                      {audit.items.slice(0, 20).map((a, i) => (
