...
function listingCardModel(listing: MarketplaceListingSummary): MarketplaceListingCardModel {
  const labels = [
  ...
  return {
    ...
    sellerLabel: statedSellerLabel(listing),
    locationLabel: summaryLocationLine(listing.location, listing.location_state).label,
    plateStatus: plateStatusLabel(listing),
    plateVerified: listing.plate_verified === true && !['flagged', 'suspended'].includes(String(listing.plate_status || '').toLowerCase()),
            ...
            {listings.length > 0 && (
              <div className="mt-8 grid gap-x-7 gap-y-12 md:grid-cols-2 xl:grid-cols-3">
                {listings.map(vehicle => (
                  <MarketplaceListingCard
                    key={vehicle.vin}
                    vehicle={listingCardModel(vehicle)}
