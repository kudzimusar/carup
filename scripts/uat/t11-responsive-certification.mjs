/**
 * Trade OS T11 — responsive certification at seven widths, on the deployed frontend.
 *
 ...
 *      being unusable.
 *
 * It also asserts the page actually RENDERED its T11 content at each width. A surface that fails to
 * load has a perfect scrollWidth, and certifying that would be certifying nothing.
 *
 * Usage:
 *   node scripts/uat/t11-responsive-certification.mjs --fe <url> --session-file <path> [--out <dir>]
 */
...
// The shipment the T11 journeys created. Passed in rather than hardcoded, because a shipment id is
// generated, not a fixture.
const SHIPMENT = args.shipment || null;
if (!SHIPMENT) { console.error('--shipment is required (the id printed by t11-shipment-journeys.mjs)'); process.exit(2); }

const SURFACES = [
  // The operator workspace is opened by typing a shipment reference, so the harness types it: the
  // certification has to exercise the LOADED state, not the empty prompt.
  { name: 'operator shipment timeline', path: '/diaspora/shipments', as: 'operator', expect: 'shipment-timeline-workspace', openShipment: SHIPMENT, settled: 'shipment-departure' },
  { name: 'participant tracking view', path: `/diaspora/tracking/${SUBJECT}`, as: 'customer', expect: 'my-shipment-tracking' },
];
  ...
  for (const [width, height] of WIDTHS) {
    ...
    try {
      ...
      if (surface.openShipment) {
        await page.fill('[data-testid="shipment-input"]', surface.openShipment);
      ...
      results.push({
        ...
        // The T11 sentences that must survive at every width. "Not recorded" is the whole phase in
        // two words: an unobserved departure is unknown, never zero and never a date.
        saysNotKnown: /Not known|Not recorded|No estimate given/i.test(visibleText),
        ...
        statesBoundary: /recorded separately|not shown here|not a record that anything has moved|not a record that it left|an estimate can move/i.test(disclaimerText),
        // 'departed', 'in transit' and 'arrived' are T11's OWN vocabulary, so the ban here is only
        // on T12 — what customs DECIDED, as opposed to where the goods are. A hold is a place.
        saysLaterPhase: /customs cleared|\bcleared customs\b|duty (paid|assessed|payable)|\bdeclaration\b|import duty|\bVAT\b|\bZIMRA\b/i.test(visibleText),
      });
