...
test.describe('Seller media continuity across the commerce lifecycle', () => {
  ...
  test('the seller-selected cover survives publish, unpublish, republish and sold', async ({ page, request }, testInfo) => {
    ...
    // Zimbabwe registration stage — a step-0 control, and now a BLOCKING publication requirement:
    // an unrecorded stage evaluates as "not established" and refuses publication, so a fixture
    // that declares nothing can no longer be published. The requirement arrived with the Zimbabwe
    // seller-reality lane, which gave the Golden fixtures a stage (64043931) but not this gate —
    // the two never met before the stack was integrated, because this gate triggers on a
    // different base. `locally_registered` is the stage consistent with the plate above and the
    // registration book this test uploads and has a reviewer verify.
