...
DO $fk$ BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.vehicles ADD CONSTRAINT vehicles_active_reservation_id_fkey
...
BEGIN
  ...
  IF v_tx.seller_id IS NULL OR v_tx.seller_id=p_actor_id THEN
    RAISE EXCEPTION 'transaction counterparty unresolved/self-referential' USING ERRCODE='23514';
  END IF;
  IF v_tx.status<>'eligible' THEN
    RAISE EXCEPTION 'transaction intent is not eligible for reservation (status=%)',v_tx.status
      USING ERRCODE='23514';
  END IF;
  IF v_tx.inquiry_id IS NULL
     OR v_tx.listing_amount IS NULL OR v_tx.listing_amount<=0
  ...
  SELECT * INTO v_inquiry
   ...
   WHERE id=v_tx.inquiry_id
     AND listing_id=v_tx.vin
  ...
  -- Idempotency is checked before creating anything, but only after the immutable transaction and
  -- inquiry lineage are authenticated. A token can never be used to discover/replay another buyer's
  -- reservation.
  ...
  IF FOUND THEN
    IF v_existing.transaction_intent_id<>p_transaction_intent_id
      ...
      RAISE EXCEPTION 'idempotency key bound to different reservation' USING ERRCODE='23505';
    END IF;
    RETURN QUERY SELECT
      v_existing.id,v_existing.vin,v_existing.transaction_intent_id,v_existing.inquiry_id,
      v_existing.status,v_existing.reserved_at,v_existing.expires_at,true;
    RETURN;
  END IF;
  ...
  -- LOAD-BEARING STALE-LINEAGE CHECKS. The application may have evaluated the intent seconds ago,
  -- but PostgreSQL is the writer that actually creates the hold. It therefore rechecks the current
  -- seller and economics under the same row lock used for the reservation. `owner_id` is never a
  -- seller fallback: Phase 4 defines current_seller_id as the governed seller relationship.
  IF nullif(btrim(v_vehicle.current_seller_id),'') IS NULL
     OR v_vehicle.current_seller_id IS DISTINCT FROM v_tx.seller_id THEN
    RAISE EXCEPTION 'listing seller changed or is no longer governed; transaction intent is stale'
      USING ERRCODE='23514';
  END IF;
  IF v_vehicle.price IS NULL OR v_vehicle.price::numeric IS DISTINCT FROM v_tx.listing_amount THEN
    RAISE EXCEPTION 'listing amount changed; transaction intent is stale' USING ERRCODE='23514';
  END IF;
  IF nullif(btrim(v_vehicle.currency),'') IS NULL
     OR upper(btrim(v_vehicle.currency)) IS DISTINCT FROM upper(btrim(v_tx.listing_currency)) THEN
    RAISE EXCEPTION 'listing currency changed; transaction intent is stale' USING ERRCODE='23514';
  END IF;
  IF nullif(btrim(v_vehicle.currency_source),'') IS NULL
     OR v_vehicle.currency_source IS DISTINCT FROM v_tx.listing_currency_source THEN
    RAISE EXCEPTION 'listing currency provenance changed; transaction intent is stale'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT v_expired_canonical AND lower(coalesce(v_vehicle.status,''))='reserved' THEN
    RAISE EXCEPTION 'legacy reserved vehicle has no canonical reservation; reconciliation required'
      USING ERRCODE='23514';
  END IF;
  ...
  INSERT INTO public.vehicle_reservations(
    vin,transaction_intent_id,inquiry_id,buyer_id,seller_id,status,
    reserved_at,expires_at,idempotency_key,created_at,updated_at
  ) VALUES(
    v_tx.vin,v_tx.id,v_inquiry.id,p_actor_id,v_tx.seller_id,'active',
    v_now,v_expires,p_idempotency_key,v_now,v_now
  ...
  RETURN QUERY SELECT
    v_created.id,v_created.vin,v_created.transaction_intent_id,v_created.inquiry_id,
    v_created.status,v_created.reserved_at,v_created.expires_at,false;
END
$reserve$;

REVOKE ALL ON FUNCTION public.issue164_reserve_vehicle_atomic(uuid,text,text)
  FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.issue164_reserve_vehicle_atomic(uuid,text,text)
  TO service_role;
